<script>
		...
		import {Navbar,NavbarBrand,Nav,NavItem,Input, Button, Popover,} from 'sveltestrap'
...
<Navbar color="primary" class="p-2 sticky-top" container>
	...
	<Nav>
		...
		<NavItem><Button id="sort">Sort</Button>
			<Popover bottom target="sort">
				<Slider {...slider} label="Name" bind:value={sort.name} />
				<Slider {...slider} label="Most Recent" bind:value={sort.last_active} />
				<Slider {...slider} label="BGG Rank" bind:value={sort.rank} />
			</Popover>
		</NavItem>	
	</Nav>
