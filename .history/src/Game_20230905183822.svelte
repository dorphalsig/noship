...
{#if visible}
  <IntersectionObserver {element} bind:intersecting threshold={0}>
    ...
    <div class={intersecting?normalClasses:undefined} bind:this={element}>
      {#if intersecting}
        <!-- HEADER -->
        <img src={game.thumbnail} class="thumbnail" alt="thumbnail" />
        <div class="name title">
          <div>
          <a href="{base_url}/{game.type}/{game.id}" target="_blank" title={game.name}>{game.name}</a> ({game.items.length} items)</div>
        </div>
...
<style>
  ...
  .offers {
    ...
    display: grid;
    grid-template-columns: 10ch min-content 140px 1fr;
